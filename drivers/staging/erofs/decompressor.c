			      ...
			      unsigned int outputsize)
{
	...
	while (cur < end) {
		...
		if (page) {
			...
			unsigned int count = min_t(uint, pageofs_out, end - cur);

			if (cur >= dst)
				memcpy(buf, cur, count);
			count = min_t(uint, righthalf, end - cur - count);
			memcpy(buf + pageofs_out, cur + pageofs_out, count);
			kunmap_atomic(buf);
		}
		cur += PAGE_SIZE;
	}
			      ...
			      bool sparsed)
{
	...
	if (nrpages_out == 1 && !overlapped) {
		...
		if (nrpages_out <= erofs_lock_pcpu_vm_area(0)) {
